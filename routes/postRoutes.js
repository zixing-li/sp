...
module.exports = (app) => {
  ...
  // @access  Public
  app.get('/api/posts/test', (req, res) => res.json({
    msg: 'Posts Works'
  }));
  ...
  // @desc    Get post by id
  // @access  Public
  app.get('/api/posts/:id', (req, res) => { // get a single post
  ...
  app.delete(
    ...
    (req, res) => {
      Profile.findOne({ // didn't add Profile model but still works, why?
        user: req.user.id
      }).then(profile => {
        Post.findById(req.params.id)
          .then(post => {
      ...
      Profile.findOne({
        ...
        Post.findById(req.params.id)
          .then(post => {
            ...
            post.likes.unshift({ // unshift will add to beginning
              user: req.user.id
            });
